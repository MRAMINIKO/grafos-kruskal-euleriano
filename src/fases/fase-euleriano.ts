...
export function iniciarFaseEuleriano(estado: EstadoJuego): void {
	...
	estado.mensaje = `Fase 2: recorré todos los túneles. Arrancás en ${estacion?.label ?? verticeInicial}.`;
}
...
export function intentarVisitarArista(
	...
	if (!esAdyacente) {
		return { aceptada: false, mensaje: "Ese túnel no sale de la estación actual." };
	}
	...
	if (estado.aristasVisitadas.size === estado.grafo.aristas.length) {
		...
		return { aceptada: true, mensaje: "¡Recorriste todos los túneles! Juego completado." };
	}
