...
import type { Escenario } from '../estado/estado-juego';

export interface ControlesHandlers {
  onReiniciar: () => void;
  onContinuar: () => void;
  onCambiarEscenario: (escenario: Escenario) => void;
}
...
export function crearControles(
  padre: HTMLElement,
  handlers: ControlesHandlers,
): PanelControles {
  ...
  tabs.style.borderBottom = '1px dashed var(--paper-border)';
  tabs.style.paddingBottom = '10px';

  const tabSubte = document.createElement('button');
  tabSubte.textContent = '🚇 Subte';
  const tabPostes = document.createElement('button');
  tabPostes.textContent = '⚡ Postes';

  for (const [tab, esc] of [
    [tabSubte, 'subte'] as const,
    [tabPostes, 'postes'] as const,
  ]) {
    tab.style.flex = '1';
    tab.style.padding = '7px 8px';
    tab.style.fontSize = '12px';
    tab.style.fontFamily = '"Special Elite", "JetBrains Mono", monospace';
    tab.style.border = '1px solid var(--paper-ink)';
    tab.style.borderRadius = '2px';
    tab.style.cursor = 'pointer';
    tab.style.letterSpacing = '0.02em';
    tab.addEventListener('click', () => handlers.onCambiarEscenario(esc));
  }
  tabs.append(tabSubte, tabPostes);
  ...
  return {
    actualizar(estado: EstadoJuego): void {
      ...
      // Detectamos el escenario para elegir el sustantivo correcto.
      const esPostes = estado.grafo.vertices.some((v) =>
        v.id.startsWith('poste'),
      );
      const sustantivo = esPostes ? 'cable' : 'túnel';
      const sustantivoPlural = esPostes ? 'cables' : 'túneles';

      const metaKruskal = estado.grafo.vertices.length - 1;
      marcarItem(
        itemKruskal,
        arbolCompleto(estado),
        `Tender la red sin ciclos (${estado.aristasSeleccionadas.size}/${metaKruskal} ${sustantivoPlural})`,
      );

      const total = estado.grafo.aristas.length;
      const enOAlPasarEuler = estado.fase !== 'kruskal';
      marcarItem(
        itemEuler,
        estado.aristasVisitadas.size === total && enOAlPasarEuler,
        enOAlPasarEuler
          ? `Inspeccionar cada ${sustantivo} una vez (${estado.aristasVisitadas.size}/${total})`
          : `Inspeccionar cada ${sustantivo} una vez (pendiente de Orden 1)`,
      );

      mensaje.textContent =
        estado.mensaje || `Tocá un ${sustantivo} del plano para empezar.`;
      ...
      botonContinuar.style.cursor = botonContinuar.disabled
        ? 'not-allowed'
        : 'pointer';
    },
    ...
    setEscenario(escenario: Escenario): void {
      ...
      const inactivo = { background: 'transparent', color: 'var(--paper-ink)' };
      Object.assign(tabSubte.style, escenario === 'subte' ? activo : inactivo);
      Object.assign(
        tabPostes.style,
        escenario === 'postes' ? activo : inactivo,
      );
    },
  };
}
