import { coloresRuta, COLOR_CABLE } from '../render/estilos-cytoscape';
...
export interface PanelLeyenda {
  actualizar(escenario: Escenario): void;
}
...
export function crearLeyenda(padre: HTMLElement): PanelLeyenda {
  ...
  for (const { color, estilo, texto } of ESTADOS) {
    estados.appendChild(
      crearFila(crearMuestraLinea(color, estilo === 'dashed'), texto),
    );
  }
  ...
  function pintarRutas(escenario: Escenario): void {
    rutas.innerHTML = '';
    if (escenario === 'subte') {
      for (const [ruta, color] of Object.entries(coloresRuta)) {
        rutas.appendChild(
          crearFila(
            crearMuestraLinea(color, false),
            NOMBRES_RUTA[ruta] ?? ruta,
          ),
        );
      }
    } else {
      rutas.appendChild(
        crearFila(crearMuestraLinea(COLOR_CABLE, false), 'Cable de tendido'),
      );
      const nota = document.createElement('div');
      nota.textContent = 'peso = distancia + dificultad';
      nota.style.fontSize = '10px';
      nota.style.color = '#8fb4e3';
      nota.style.marginTop = '4px';
      rutas.appendChild(nota);
    }
  }

  pintarRutas('subte');

  return { actualizar: pintarRutas };
}
