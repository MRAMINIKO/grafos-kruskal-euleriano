...
import {
  ...
  reiniciarEstado,
  cambiarEscenario,
} from './estado/estado-juego';
import { intentarSeleccionarArista, arbolCompleto } from './fases/fase-kruskal';
import {
  intentarVisitarArista,
  iniciarFaseEuleriano,
} from './fases/fase-euleriano';
...
const leyenda = crearLeyenda(document.body);
...
const controles = crearControles(document.body, {
  ...
  onCambiarEscenario: (escenario) => {
    cambiarEscenario(escenario);
    const datos =
      escenario === 'subte'
        ? { v: vertices, a: aristas }
        : { v: verticesPostes, a: aristasPostes };
    cargarGrafo(cy, datos.v, datos.a);
    controles.setEscenario(escenario);
    leyenda.actualizar(escenario);
    refrescar();
  },
});

controles.setEscenario('subte');
